...
public class DataProviderJdbc implements DataProvider {
  ...
  public boolean editFursuitPart(long userId, FursuitPart editedFursuitPart) {
    try {
      ...
      executesRequest(String.format(Locale.ENGLISH, ConfigurationUtil.getConfigurationEntry(Constants.UPDATE_FURSUIT_PART_REQUEST),
              editedFursuitPart.getName(),
              editedFursuitPart.getCost(),
  ...
  public boolean deleteFursuitPart(long userId, long projectId, long partId) {
    try {
      ...
      executesRequest(String.format(ConfigurationUtil.getConfigurationEntry(Constants.DELETE_FURSUIT_PART_REQUEST),
              partId,
              userId,
              projectId));
  ...
  private FursuitPart setFursuitPart(ResultSet resultSet) {
    try {
      ...
      fursuitPart.setCost(resultSet.getFloat(ConfigurationUtil.getConfigurationEntry(Constants.COLUMN_LABEL_FURSUIT_PART_COST)));
      return fursuitPart;
    } catch (Exception e) {
      log.error(e);
